...
const RIGHT_FREQUENCY = 0.55;
const FREQUENCY_EPSILON = 0.1;
...
export default function Radio({ onComplete, windowWidth }: { onComplete: () => void, windowWidth: number }) {
  ...
  function onKnobRotate(delta: number) {
    ...
    setIsRightFrequency(frequencyRef.current > RIGHT_FREQUENCY - FREQUENCY_EPSILON && frequencyRef.current < RIGHT_FREQUENCY + FREQUENCY_EPSILON);
  ...
  return (
    <div ref={containerRef} className="h-full w-full flex justify-center items-center select-none">
      ...
      />
      <div
        className={`absolute cursor-grab ${isMobile ? 'w-[14%] mt-[22%] ml-[-37%]' : 'w-[11%] mt-[17.8%] ml-[-29.5%]'} hover:scale-105`}
        ref={leftKnobRef}
      >
        <img className="pointer-events-none" src={ImgRadioKnob} alt="radio knob" />
      </div>
      <div
        className={`absolute cursor-grab ${isMobile ? 'w-[14%] mt-[22%] mr-[-38%]' : 'w-[11%] mt-[17.8%] mr-[-30.5%]'} hover:scale-105`}
        ref={rightKnobRef}
      >
        <img className="pointer-events-none" src={ImgRadioKnob} alt="radio knob" />
      </div>
